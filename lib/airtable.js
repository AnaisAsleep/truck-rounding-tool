 ...
 */
const PALLETIZATION_FIELDS = [
  'pkey', 'completeness', 'category',
  'origin_location_record', 'origin_location_name',
  'loading_unit', 'sku',
  'pcs_per_ftl_container', 'pcs_per_pallet', 'pallets_per_truck',
  'loading_type', 'receiving_destination_record', 'lane',
];

const COST_FIELDS = [
  'lane_type', 'mode_of_transportation',
  'origin_location', 'origin_supplier_name', 'destination_location',
  'carrier_code', 'transport_cost_total_eur',
  'active_status', 'start_date', 'end_date', 'lane',
];

export async function fetchPalletizationTable() {
  // Fetch only needed fields to reduce response size and speed up pagination
  const records = await fetchAllRecords(PALLETIZATION_TABLE_ID, { fields: PALLETIZATION_FIELDS });
...
export async function fetchCostTable() {
  // Fetch only needed fields to reduce response size and speed up pagination
  const records = await fetchAllRecords(COST_TABLE_ID, { fields: COST_FIELDS });
