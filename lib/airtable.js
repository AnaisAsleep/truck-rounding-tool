...
async function fetchAllRecords(tableId, params = {}) {
  ...
  do {
    ...
    const response = await fetch(url, {
      ...
      // Cache each Airtable page for 1 hour on Vercel's edge.
      // The "Refresh" button in SetupStep bypasses this by calling /api/airtable
      // with cache: 'no-store' from the browser — so users can still force a refresh.
