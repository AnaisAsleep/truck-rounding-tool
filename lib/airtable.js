...
async function fetchAllRecords(tableId, params = {}, retries = 3) {
    ...
    try {
      ...
      do {
        ...
        if (!response.ok) {
          ...
          // Offset expired — restart the whole fetch from page 1
          if (response.status === 422 && errorText.includes('LIST_RECORDS_ITERATOR_NOT_AVAILABLE')) {
            throw Object.assign(new Error('ITERATOR_EXPIRED'), { retryable: true });
          }
      ...
      if (err.retryable && attempt < retries - 1) continue;
      throw err;
    }
  }
}
...
export async function fetchPalletizationTable() {
  const records = await fetchAllRecords(PALLETIZATION_TABLE_ID, {
    filterByFormula: "AND({category}='PO',OR({loading_unit}='FTL',{loading_unit}='CONTAINER 40FT'))",
    fields: [
...
export async function fetchCostTable() {
  const records = await fetchAllRecords(COST_TABLE_ID, {
    filterByFormula: "{active_status}='Active'",
    fields: [
