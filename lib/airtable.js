...
export async function fetchPalletizationTable() {
  // Only fetch rows where loading_unit is FTL or CONTAINER 40FT
  // Airtable formula: OR({loading_unit}='FTL', {loading_unit}='CONTAINER 40FT')
  const records = await fetchAllRecords(PALLETIZATION_TABLE_ID, {
    filterByFormula: "OR({loading_unit}='FTL', {loading_unit}='CONTAINER 40FT')",
  });
...
export async function fetchCostTable() {
  // Only fetch active cost records
