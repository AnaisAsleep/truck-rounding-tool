...
export async function fetchCostTable() {
  ...
  return records
    .map(record => {
      const f = record.fields;
      const originCode  = String(f['origin_location_code']      || f['origin_location']      || '').trim();
      const destCode    = String(f['destination_location_code'] || f['destination_location'] || '').trim();
      // Always derive the lane key from actual location codes so it matches what
      // excelParser builds from the needs file (originLocationCode|destinationLocation).
      // The Airtable 'lane' formula field may be empty or use a different separator.
      const derivedLane = originCode && destCode ? `${originCode}|${destCode}` : String(f['lane'] || '').trim();
      return {
        id: record.id,
        lane: derivedLane,
        ...
        destination_location_code: destCode,
...
export async function fetchLocationLookupTable() {
  ...
  do {
    ...
    if (offset) query.set('offset', offset);
    query.set('pageSize', '100');
    ['id_vendor_emm (from vendor_code_link)', 'vendor_code', 'location_code'].forEach(f => query.append('fields[]', f));
  ...
  for (const record of records) {
    ...
    // id_vendor_emm (from vendor_code_link) is a lookup field — comes back as an array
    const idVendorEmm = Array.isArray(f['id_vendor_emm (from vendor_code_link)'])
      ? f['id_vendor_emm (from vendor_code_link)'][0]
      : f['id_vendor_emm (from vendor_code_link)'];
