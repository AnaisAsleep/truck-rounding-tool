...
function deriveAutoRootCause(truck) {
  const fill = Math.round((truck.usedFraction || 0) * 100);
  const cpp = truck.costPerPiece;
  if (cpp != null && cpp > 20) return `High cost per piece — €${cpp.toFixed(2)}/pc exceeds €20 threshold`;
  if (cpp != null && cpp >= 10) return `Borderline cost per piece — €${cpp.toFixed(2)}/pc (€10–€20 range)`;
  if (fill < 50) return `Low fill rate — ${fill}% (below 50% minimum)`;
  if (fill < 80) return `Insufficient volume — ${fill}% fill (below 80% target)`;
  return truck.cutReason || 'Algorithm decision';
}
...
/**
 * Build pass-through trucks for MI_PT / VELA_ES suppliers.
 *
 * These suppliers require exact input quantities — no pallet rounding,
 * no truck capacity limits, no Prio 4 top-up. We simply group all their
 * rows by lane + PGRD week and emit one truck per group, preserving every
 * prio/SKU line exactly as received.
 *
 * All resulting trucks are auto-confirmed and get PT_ VSNs.
 */
function buildPassThroughTrucks(rows) {
  const groups = {};
  ...
  const trucks = [];
  for (const [, laneRows] of Object.entries(groups)) {
    const origin = laneRows[0].originLocationCode;
    const destination = laneRows[0].destinationLocation;
    const pgrdYear = laneRows[0].pgrdYear || '';
    const pgrdWeek = laneRows[0].pgrdWeek || '';
    const lines = [];
    let minPrio = 9;

    for (const row of laneRows) {
      for (const [prio, qty] of [[1, row.prio1], [2, row.prio2], [3, row.prio3]]) {
        if (qty > 0) {
          // Compute pallets for display only — not used for capacity
          const pallets = row.palletData?.pcs_per_pallet > 0
            ? Math.floor(qty / row.palletData.pcs_per_pallet) : 0;
          lines.push({
            sku: row.sku,
            ...
            pallets,
            priority: prio,
            loadingType: row.palletData?.loading_type || '',
            loadingUnit: row.palletData?.loading_unit || 'FTL',
            lineFraction: 0, // N/A — no capacity constraint
            palletData: row.palletData,
            lane: row.lane,
    ...
    trucks.push({
      origin,
      destination,
      lane: laneRows[0].lane,
      pgrdYear,
      pgrdWeek,
      lines,
      usedFraction: 1.0, // Treat as 100% — exact S&OP input, no capacity concept
      minPrio: minPrio === 9 ? 1 : minPrio,
      vendorShipmentNumber: null, // Assigned in assignVendorShipmentNumbers
      decision: 'Pass-Through (exact quantities)',
      cutReason: null,
      isPassThrough: true,
    });
  }
...
export function assignVendorShipmentNumbers(trucks, weekNum, year) {
  // First pass: build base VSNs — origin is included so each supplier has its own namespace
  const withBase = trucks.map(truck => {
    ...
    const origin = truck.origin || truck.lines[0]?.originLocationCode || '';
    const base = origin ? `${origin}_${typeCode}_P${minPrio}${pgrdSuffix}` : `${typeCode}_P${minPrio}${pgrdSuffix}`;
    return { ...truck, _baseVSN: base };
  });

  // Second pass: disambiguate duplicates within the same origin by appending _2, _3, etc.
  const seen = {};
...
// ============================================================
// SECTION 5b: Milk Run Detection & Finalization
// ============================================================

/**
 * Extract the 2-character country code from a warehouse destination code.
 * Convention: last segment after '_'. e.g. HA_DE→DE, KNR1_DE→DE, DSB1_ES→ES.
 * Returns null if the pattern doesn't match or the segment isn't 2 characters.
 */
export function getCountryCode(destination) {
  const parts = String(destination || '').split('_');
  if (parts.length < 2) return null;
  const cc = parts[parts.length - 1];
  return cc.length === 2 ? cc.toUpperCase() : null;
}

/** Strip country suffix for the VSN: 'HA_DE' → 'HA', 'KNR1_DE' → 'KNR1' */
function getDestShortCode(destination) {
  const parts = String(destination || '').split('_');
...
/**
 * Detect milk run candidates from the current round's trucks.
 *
 * A milk run groups two same-supplier, same-country, same-PGRD-week destinations
 * into one physical truck that makes two stops.
 *
 * Two detection paths:
 *   1. Pure remainders — cutTrucks + borderlineTrucks with 2+ same-country destinations
 *      from the same supplier. If >2 available, top 2 by volume form the milk run.
 *   2. Upgrades — a confirmed truck with meaningful remaining capacity can absorb
 *      a same-country sub-threshold stop (≥20% fill contribution).
 *
 * Eligibility (all must be true):
 *   - Combined fill ≤ 100% (physically fits)
 *   - Second stop contributes ≥ 20% fill
 *   - Max 2 destination stops
 *
 * @param {Array} cutTrucks
 * @param {Array} borderlineTrucks
 * @param {Array} confirmedTrucks
 * @returns {{ candidates: Array, consumedVSNs: Set<string> }}
 */
export function detectMilkRunCandidates(cutTrucks, borderlineTrucks, confirmedTrucks) {
  const subthresholdTrucks = [
    ...cutTrucks.map(t => ({ ...t, _mrSource: 'cut' })),
    ...borderlineTrucks.map(t => ({ ...t, _mrSource: 'borderline' })),
  ];

  const consumedVSNs = new Set();
  const candidates = [];
  let milkRunN = 1;

  // ── Phase 1: Pure remainder candidates ──────────────────────────────
  const byGroup = {};
  for (const truck of subthresholdTrucks) {
    const cc = getCountryCode(truck.destination);
    if (!cc) continue; // Non-standard code — log in spec section 6
    const key = `${truck.origin}|${cc}|${truck.pgrdYear || ''}|${truck.pgrdWeek || ''}`;
    if (!byGroup[key]) {
      byGroup[key] = {
        origin: truck.origin, country: cc,
        pgrdYear: truck.pgrdYear || '', pgrdWeek: truck.pgrdWeek || '',
        trucks: [],
      };
    }
    byGroup[key].trucks.push(truck);
  }

  for (const group of Object.values(byGroup)) {
    const byDest = {};
    for (const truck of group.trucks) {
      if (!byDest[truck.destination]) byDest[truck.destination] = [];
      byDest[truck.destination].push(truck);
    }
    if (Object.keys(byDest).length < 2) continue;

    const destVolumes = Object.entries(byDest).map(([dest, trucks]) => ({
      dest,
      trucks,
      totalFill: trucks.reduce((s, t) => s + (t.usedFraction || 0), 0),
      lines: trucks.flatMap(t => t.lines || []),
    })).sort((a, b) => b.totalFill - a.totalFill);

    const top2 = destVolumes.slice(0, 2);
    const combinedFill = top2.reduce((s, dv) => s + dv.totalFill, 0);
    if (combinedFill > 1.0 || top2[1].totalFill < 0.20) continue;

    const primaryTruck = top2[0].trucks[0];
    const loadingUnit = primaryTruck.lines?.[0]?.loadingUnit ||
      primaryTruck.lines?.[0]?.palletData?.loading_unit || 'FTL';
    const typeCode = loadingUnit === 'CONTAINER 40FT' ? 'S40FT' : 'FTL';
    const allLines = top2.flatMap(dv => dv.lines);
    const totalPieces = allLines.reduce((s, l) => s + (l.qty || 0), 0);
    const costRecord = primaryTruck.lines?.[0]?.costData || null;
    const costPerPiece = costRecord?.transport_cost_total_eur && totalPieces > 0
      ? costRecord.transport_cost_total_eur / totalPieces : null;

    top2.flatMap(dv => dv.trucks.map(t => t.vendorShipmentNumber))
      .forEach(vsn => consumedVSNs.add(vsn));

    candidates.push({
      milkRunId: `MR${milkRunN++}`,
      origin: group.origin,
      country: group.country,
      pgrdWeek: group.pgrdWeek,
      pgrdYear: group.pgrdYear,
      loadingUnit,
      typeCode,
      stops: top2.map(dv => ({
        destination: dv.dest,
        shortDest: getDestShortCode(dv.dest),
        lines: dv.lines,
        fillFraction: dv.totalFill,
        sourceTrucks: dv.trucks.map(t => t.vendorShipmentNumber),
      })),
      totalFillFraction: combinedFill,
      costPerPiece,
      transportCost: costRecord?.transport_cost_total_eur || null,
      totalPieces,
      isUpgrade: false,
      upgradedFromVSN: null,
    });
  }

  // ── Phase 2: Upgrade candidates ──────────────────────────────────────
  const remainingSub = subthresholdTrucks.filter(t => !consumedVSNs.has(t.vendorShipmentNumber));

  for (const confirmed of confirmedTrucks) {
    if (confirmed.isPassThrough || confirmed.isMilkRun) continue;
    if (consumedVSNs.has(confirmed.vendorShipmentNumber)) continue;
    const remainingCapacity = 1.0 - (confirmed.usedFraction || 0);
    if (remainingCapacity < 0.20) continue;

    const cc = getCountryCode(confirmed.destination);
    if (!cc) continue;

    const target = remainingSub.find(t =>
      !consumedVSNs.has(t.vendorShipmentNumber) &&
      t.origin === confirmed.origin &&
      t.pgrdYear === (confirmed.pgrdYear || '') &&
      t.pgrdWeek === (confirmed.pgrdWeek || '') &&
      t.destination !== confirmed.destination &&
      getCountryCode(t.destination) === cc &&
      (t.usedFraction || 0) >= 0.20 &&
      (confirmed.usedFraction || 0) + (t.usedFraction || 0) <= 1.0
    );
    if (!target) continue;

    consumedVSNs.add(target.vendorShipmentNumber);
    consumedVSNs.add(confirmed.vendorShipmentNumber);

    const loadingUnit = confirmed.lines?.[0]?.loadingUnit ||
      confirmed.lines?.[0]?.palletData?.loading_unit || 'FTL';
    const typeCode = loadingUnit === 'CONTAINER 40FT' ? 'S40FT' : 'FTL';
    const allLines = [...(confirmed.lines || []), ...(target.lines || [])];
    const totalPieces = allLines.reduce((s, l) => s + (l.qty || 0), 0);
    const costRecord = confirmed.lines?.[0]?.costData || null;
    const costPerPiece = costRecord?.transport_cost_total_eur && totalPieces > 0
      ? costRecord.transport_cost_total_eur / totalPieces : null;
    const combinedFill = (confirmed.usedFraction || 0) + (target.usedFraction || 0);

    candidates.push({
      milkRunId: `MR${milkRunN++}`,
      origin: confirmed.origin,
      country: cc,
      pgrdWeek: confirmed.pgrdWeek || '',
      pgrdYear: confirmed.pgrdYear || '',
      loadingUnit,
      typeCode,
      stops: [
        {
          destination: confirmed.destination,
          shortDest: getDestShortCode(confirmed.destination),
          lines: confirmed.lines || [],
          fillFraction: confirmed.usedFraction || 0,
          sourceTrucks: [confirmed.vendorShipmentNumber],
        },
        {
          destination: target.destination,
          shortDest: getDestShortCode(target.destination),
          lines: target.lines || [],
          fillFraction: target.usedFraction || 0,
          sourceTrucks: [target.vendorShipmentNumber],
        },
      ],
      totalFillFraction: combinedFill,
      costPerPiece,
      ...
      totalPieces,
      isUpgrade: true,
      upgradedFromVSN: confirmed.vendorShipmentNumber,
    });
  }

  return { candidates, consumedVSNs };
}

/**
 * Convert milk run decisions (from MilkRunStep) into confirmed truck objects
 * and cut line objects.
 *
 * Decision shape per milkRunId:
 *   { action: 'approve'|'approve_lu'|'20ft'|'cut', luReason?, freeText?, stopsToCut: string[] }
 *
 * @param {Array}  milkRunCandidates
 * @param {Object} decisions  — map of milkRunId → decision object
 * @param {number} weekNum
 * @returns {{ confirmedMilkRunTrucks: Array, milkRunCutLines: Array }}
 */
export function finalizeMilkRunDecisions(milkRunCandidates, decisions, weekNum) {
  const CONTAINER_20FT_FRACTION = 0.45;
  const confirmedMilkRunTrucks = [];
  const milkRunCutLines = [];

  for (const mr of milkRunCandidates) {
    const d = decisions[mr.milkRunId] || { action: 'cut' };
    const { action, luReason = null, freeText = null } = d;
    const stopsToCut = new Set(d.stopsToCut || []);

    if (action === 'cut') {
      for (const stop of mr.stops) {
        for (const line of stop.lines) {
          milkRunCutLines.push({
            originLocationCode: line.originLocationCode || mr.origin,
            supplierName: line.supplierName || '',
            destinationLocation: stop.destination,
            sku: line.sku,
            originalQty: line.qty,
            priority: line.priority,
            lane: `${mr.origin}|${stop.destination}`,
            cutReason: 'Milk run cut by planner',
            transportUnitType: mr.typeCode === 'S40FT' ? '40ft Container' : 'FTL',
            fallbackUnit: null,
            rootCause: freeText,
            costPerPiece: mr.costPerPiece,
            fillPct: stop.fillFraction,
            isMilkRunCut: true,
          });
        }
      }
    ...
    // Build VSN prefix components
    const luSuffix = luReason ? `_${luReason}` : '';
    const typeCode = action === '20ft' ? 'S20FT' : mr.typeCode;
    const pgrdStr = mr.pgrdWeek || weekNum;
    const milkRunGroupId = `${mr.milkRunId}_${typeCode}${luSuffix}_PW${pgrdStr}_${mr.origin}_${mr.country}`;

    for (const stop of mr.stops) {
      if (stopsToCut.has(stop.destination)) {
        for (const line of stop.lines) {
          milkRunCutLines.push({
            originLocationCode: line.originLocationCode || mr.origin,
            supplierName: line.supplierName || '',
            ...
            priority: line.priority,
            lane: `${mr.origin}|${stop.destination}`,
            cutReason: 'Originally proposed as milk run stop — insufficient volume for standalone shipment',
            transportUnitType: mr.typeCode === 'S40FT' ? '40ft Container' : 'FTL',
            fallbackUnit: null,
            rootCause: freeText,
            costPerPiece: mr.costPerPiece,
      ...
      const stopFill = action === '20ft'
        ? Math.min(stop.fillFraction / CONTAINER_20FT_FRACTION, 1)
        : stop.fillFraction;
      const vsn = `${mr.milkRunId}_${typeCode}${luSuffix}_PW${pgrdStr}_${mr.origin}_${mr.country}_${stop.shortDest}`;
      const priorities = stop.lines.map(l => l.priority).filter(p => p >= 1 && p <= 3);
      const minPrio = priorities.length > 0 ? Math.min(...priorities) : 1;

      confirmedMilkRunTrucks.push({
        origin: mr.origin,
        destination: stop.destination,
        lane: `${mr.origin}|${stop.destination}`,
        pgrdYear: mr.pgrdYear,
        pgrdWeek: mr.pgrdWeek,
        lines: stop.lines,
        usedFraction: stopFill,
        totalFillFraction: action === '20ft'
          ? Math.min(mr.totalFillFraction / CONTAINER_20FT_FRACTION, 1)
          : mr.totalFillFraction,
        minPrio,
        vendorShipmentNumber: vsn,
        milkRunGroupId,
        milkRunId: mr.milkRunId,
        milkRunStop: stop.shortDest,
        milkRunAllStops: mr.stops.map(s => s.destination).join(' + '),
        isMilkRun: true,
        decision: action === '20ft' ? `Milk run — rebooked 20ft (${luReason})`
          : luReason ? `Milk run — approved (${luReason})`
          : 'Milk run — approved',
        luReason,
        luNote: freeText || null,
        transportUnitOverride: action === '20ft' ? '20ft Container' : null,
        transportMode: mr.typeCode === 'S40FT' ? 'sea' : 'road',
        isUpgrade: mr.isUpgrade,
        upgradedFromVSN: mr.upgradedFromVSN,
        costPerPiece: (() => {
          if (action !== '20ft') return mr.costPerPiece;
          const cost20ftRecord = stop.lines?.[0]?.cost20ftData;
          if (cost20ftRecord?.transport_cost_total_eur != null && mr.totalPieces > 0) {
            return cost20ftRecord.transport_cost_total_eur / mr.totalPieces;
          }
          return mr.costPerPiece != null ? mr.costPerPiece * CONTAINER_20FT_FRACTION : null;
        })(),
        transportCost: (() => {
          if (action !== '20ft') return mr.transportCost;
          const cost20ftRecord = stop.lines?.[0]?.cost20ftData;
          if (cost20ftRecord?.transport_cost_total_eur != null) return cost20ftRecord.transport_cost_total_eur;
          return mr.transportCost != null ? mr.transportCost * CONTAINER_20FT_FRACTION : null;
        })(),
        totalPieces: mr.totalPieces,
      });
    }
  }

  return { confirmedMilkRunTrucks, milkRunCutLines };
}
 ...
 * }}
 */
// Fennobed MTS allowlist — exported so the UI can show MTO/MTS labels at review time
export const FENN_SUPPLIERS = new Set(['FENN_EE', 'FEN1_EE']);
export const FENN_MTS_SKUS = new Set([
...
/** Returns 'MTO', 'MTS', or null depending on supplier and SKU */
export function getMtoMtsLabel(originLocationCode, sku) {
  if (!FENN_SUPPLIERS.has(originLocationCode)) return null;
  return FENN_MTS_SKUS.has(sku) ? 'MTS' : 'MTO';
}

export function runRounding(validRows, prio4Rows, costMap, weekNum, year, isBedsAndAcc = false) {
  // Suppliers that require exact pass-through (no rounding, no prio4, no decision rules)
  const PASSTHROUGH_SUPPLIERS = new Set(['MI_PT', 'VELA_ES']);

  // ── Separate pass-through rows from normal rows ─────────────────────
  const passThroughRows = validRows.filter(r => PASSTHROUGH_SUPPLIERS.has(r.originLocationCode));
  const normalRows = validRows.filter(r => !PASSTHROUGH_SUPPLIERS.has(r.originLocationCode));

  // ── Step 0: Group normal rows by lane + PGRD week ───────────────────
  ...
  // Build prio4 index — pass-through suppliers never get top-up;
  // FENN MTO SKUs cannot use Index 1 for HA_DE destination.
  const prio4ByLane = {};
  for (const row of (prio4Rows || [])) {
    if (PASSTHROUGH_SUPPLIERS.has(row.originLocationCode)) continue;
    if (FENN_SUPPLIERS.has(row.originLocationCode)) {
      const isMts = FENN_MTS_SKUS.has(row.sku);
      // MTO SKUs are blocked for HA_DE; allowed for any other destination
      if (!isMts && row.destinationLocation === 'HA_DE') continue;
    }
  ...
  let milkRunCandidates = [];
  let milkRunConsumedVSNs = new Set();

  if (isBedsAndAcc) {
    const mrResult = detectMilkRunCandidates(cutTrucks, borderlineTrucks, confirmedTrucks);
    milkRunCandidates = mrResult.candidates;
    milkRunConsumedVSNs = mrResult.consumedVSNs;
  }

  // Cut lines from explicitly cut trucks — skip trucks consumed by milk runs
  for (const truck of cutTrucks) {
    if (milkRunConsumedVSNs.has(truck.vendorShipmentNumber)) continue;
  ...
  // ── Step 5: Build and auto-confirm pass-through trucks ─────────────
  let ptTrucks = buildPassThroughTrucks(passThroughRows);
  ptTrucks = assignVendorShipmentNumbers(ptTrucks, weekNum, year);
  const passThroughConfirmed = ptTrucks.map(t => ({
    ...t,
    transportMode: 'road',
  }));
  ...
  return {
    confirmedTrucks: [
      // Remove confirmed trucks consumed by milk run upgrades
      ...confirmedTrucks.filter(t => !milkRunConsumedVSNs.has(t.vendorShipmentNumber)),
      ...passThroughConfirmed,
    ],
    borderlineTrucks: borderlineTrucks.filter(t => !milkRunConsumedVSNs.has(t.vendorShipmentNumber)),
    cutTrucks: cutTrucks.filter(t => !milkRunConsumedVSNs.has(t.vendorShipmentNumber)),
    milkRunCandidates,
...
export function finalizeResults(roundingResults, userDecisions, _forceKeptVSNs, transportModeDecisions, cutLineNotes = {}, unmatchedRows = [], truckAdditions = {}) {
  ...
  const keepTruck = (truck, action, luReason, label) => {
    ...
    const minPrio = truck.minPrio === 9 ? 1 : truck.minPrio;
    const pgrdSuffix = truck.pgrdWeek ? `_PW${truck.pgrdWeek}` : '';
    // luReason is already 'LU1', 'LU2' etc — don't prepend 'LU' again
    const luSuffix = luReason ? `_${luReason}` : '_LU';
    ...
    // For 20ft rebooks, store the fill as a fraction of the 20ft capacity
    const finalFill = action === '20ft'
      ? Math.min(truck.usedFraction / CONTAINER_20FT_FRACTION, 1)
      : truck.usedFraction;
    ...
    finalConfirmed.push({
      ...
      transportUnitOverride: action === '20ft' ? '20ft Container' : null,
      usedFraction: finalFill,
      transportCost: (() => {
        if (action !== '20ft') return truck.transportCost;
        const cost20ftRecord = truck.lines?.[0]?.cost20ftData;
        if (cost20ftRecord?.transport_cost_total_eur != null) return cost20ftRecord.transport_cost_total_eur;
        return truck.transportCost != null ? truck.transportCost * CONTAINER_20FT_FRACTION : null;
      })(),
      costPerPiece: (() => {
        if (action !== '20ft') return truck.costPerPiece;
        const cost20ftRecord = truck.lines?.[0]?.cost20ftData;
        const totalPieces = linesWithNotes.reduce((s, l) => s + (l.qty || 0), 0);
        if (cost20ftRecord?.transport_cost_total_eur != null && totalPieces > 0) {
          return cost20ftRecord.transport_cost_total_eur / totalPieces;
        }
        return truck.costPerPiece != null ? truck.costPerPiece * CONTAINER_20FT_FRACTION : null;
      })(),
    });
  };

  // Helper: add truck's lines to cut output
  const cutTruck = (truck, cutReasonOverride) => {
    const rootCause = deriveAutoRootCause(truck);
    for (const line of truck.lines) {
      finalCutLines.push({
        originLocationCode: line.originLocationCode,
        supplierName: line.supplierName,
  ...
  for (const truck of borderlineTrucks) {
    ...
    if (d.action === 'keep') {
      ...
      // 'cut' or default
      cutTruck(truck, `User rejected — borderline${costLabel}`);
    }
  }
  ...
  for (const truck of cutTrucks) {
    ...
    if (d.action === 'keep') {
      ...
      cutTruck(truck, truck.cutReason);
    }
  }
