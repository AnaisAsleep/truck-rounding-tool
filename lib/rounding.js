...
export function calcFallbackUnit(qty, pallets, palletData) {
  ...
  if (!isLooseLoaded) {
    ...
    if (pallets <= VAN_PALLET_CAPACITY) return 'Van';
    if (pallets <= container20ftPallets) return '20ft Container';
    return null;
  } else {
    // Loose loaded: compare in pieces
    // Van capacity ≈ 8/pallets_per_truck fraction of the full FTL (loose loaded trucks
    // typically still have a reference pallet count for capacity estimation)
    const palletsPerTruck = palletData.pallets_per_truck || 33; // standard 13.6m trailer
    ...
    if (qty <= vanCapacityPcs) return 'Van';
...
export function assignVendorShipmentNumbers(trucks, weekNum, year) {
  ...
  return trucks.map(truck => {
    globalCounter += 1;
    const seq = String(globalCounter).padStart(2, '0');
    const minPrio = truck.minPrio === 9 ? 1 : truck.minPrio;
    const loadingUnit = truck.lines[0]?.loadingUnit || 'FTL';
    const typeCode = loadingUnit === 'CONTAINER 40FT' ? 'S40FT' : 'FTL';

    return {
      ...truck,
      vendorShipmentNumber: `${yy}${ww}-${typeCode}_P${minPrio}_${seq}`,
    };
  });
}
 ...
 * @param {Object} userDecisions - Map of vendorShipmentNumber → { action: 'keep'|'cut', rootCause: string|null }
 * @param {Array} forceKeptVSNs - VSNs the user force-kept in the Override step
 * @returns {{ finalConfirmed, finalCutLines }}
 */
export function finalizeResults(roundingResults, userDecisions, forceKeptVSNs, transportModeDecisions) {
  ...
  for (const truck of borderlineTrucks) {
    ...
    // decision shape: { action: 'keep'|'cut', rootCause: string|null }
    const action = decision?.action ?? 'cut'; // default cut if no decision
    const rootCause = decision?.rootCause ?? null;
    ...
    if (action === 'keep') {
      ...
      // User chose to cut (or default cut) — attach root cause to each cut line
      for (const line of truck.lines) {
        finalCutLines.push({
          originLocationCode: line.originLocationCode,
          supplierName: line.supplierName,
  ...
  if (transportModeDecisions) {
    for (const truck of finalConfirmed) {
      ...
      if (decision?.mode === 'rail' && decision.railReason) {
        // Rail container: replace S40FT with R_{reason} in VSN
        truck.vendorShipmentNumber = truck.vendorShipmentNumber.replace(/-S40FT_/, `-R_${decision.railReason}_`);
        truck.transportMode = 'rail';
        truck.railReason = decision.railReason;
      } else {
        // 'sea' for containers, 'road' for FTL — VSN unchanged
        truck.transportMode = decision?.mode || 'road';
      }
    }
  }
