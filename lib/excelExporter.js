...
// ─── Fennobed MTO/MTS classification ─────────────────────────────────────────

const FENN_SUPPLIERS_SET = new Set(['FENN_EE', 'FEN1_EE']);

// MTS SKUs — Made to Stock; all other FENN SKUs are Made to Order (MTO)
const FENN_MTS_SKUS_SET = new Set([
...
function getMtoMtsLabel(originLocationCode, sku) {
  if (!FENN_SUPPLIERS_SET.has(originLocationCode)) return '';
  return FENN_MTS_SKUS_SET.has(sku) ? 'MTS' : 'MTO';
}
...
export async function exportConfirmedLoads(confirmedTrucks, weekNum) {
  ...
  for (const truck of confirmedTrucks) {
    ...
    // Transport mode label (truck-level, same for all lines)
    const transportMode = truck.transportMode === 'rail' ? 'Rail'
      : truck.transportMode === 'sea' ? 'Sea'
      : 'Road';

    for (const line of truck.lines) {
      // Resolve transport unit per line (override takes precedence for Van/20ft decisions)
...
export async function generateBase64Blobs(confirmedTrucks, cutLines, weekNum) {
  ...
  for (const truck of confirmedTrucks) {
    ...
    const costPerPiece = truck.costPerPiece != null ? `€${truck.costPerPiece.toFixed(2)}` : '';
    const transportMode = truck.transportMode === 'rail' ? 'Rail' : truck.transportMode === 'sea' ? 'Sea' : 'Road';
    for (const line of truck.lines) {
      const baseTransportUnit = line.loadingUnit === 'CONTAINER 40FT' ? '40ft Container' : 'FTL';
      const transportUnitType = truck.transportUnitOverride || baseTransportUnit;
      confirmedRows.push({
