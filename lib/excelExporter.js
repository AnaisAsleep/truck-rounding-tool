 ...
 * New column: Transport Unit Type (FTL or 40ft Container).
 *
...
export function exportConfirmedLoads(confirmedTrucks, weekNum) {
  ...
  for (const truck of confirmedTrucks) {
    ...
    for (const line of truck.lines) {
      // Determine transport unit type for this line
 ...
 *
 * One row per cut quantity. Includes:
 * - Cut Reason: algorithmic reason why it was cut
 * - Root Cause: user-selected business reason (only for user-driven cuts)
 * - Suggested Fallback Unit: Van or 20ft Container if the quantity fits
 * - Transport Unit Type: original planned unit type
 *
