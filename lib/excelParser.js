...
export async function parseNeedsFile(arrayBuffer, isPrio4 = false) {
  ...
  for (let i = headerRowIndex + 1; i < rawData.length; i++) {
    ...
    // constraint=null → no cap; constraint=0 → cap to 0 (order nothing)
  ...
  // Warn if no constraint column was found — quantities will be uncapped
  if (!isPrio4 && constraintColIdx === undefined) {
    parseErrors.unshift({
      message: 'No constraint column found — quantities are uncapped',
      details: `Add a column named "Constrained quantity" (or "Constraint", "P") to cap each SKU's ordered quantity. Without it, the full Prio 1+2+3 sum is used.`,
      severity: 'warning',
    });
  }
