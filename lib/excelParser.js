...
export async function parseNeedsFile(arrayBuffer, isPrio4 = false, applyConstraint = true, locationLookup = {}) {
  ...
  // If no origin column in the file but we have a locationLookup, we can still proceed
  // (origin will be resolved row-by-row from From Whouse). Only hard-block if both are absent.
  const hasLookupFallback = Object.keys(locationLookup).length > 0;
  if (!hasOriginCode && !hasLookupFallback) {
  ...
  for (let i = headerRowIndex + 1; i < rawData.length; i++) {
    ...
    // Resolve origin location code: file column first, then From Whouse → Airtable lookup
    let originLocationCode = String(getCell(row, REQUIRED_HEADERS.originLocationCode)).trim();
    if (!originLocationCode && fromWhouse && Object.keys(locationLookup).length > 0) {
      originLocationCode = locationLookup[fromWhouse] || '';
    }
    ...
    if (!originLocationCode) {
      parseErrors.push({
        message: `Row ${i + 1}: could not resolve origin location code for SKU ${sku} (From Whouse: "${fromWhouse}")`,
        sku, destinationLocation,
        reason: 'Missing origin_location_code and no Airtable match for From Whouse — line skipped',
      });
      continue;
    }
...
export function validateRows(rows, palletizationData, costData, cost20ftMap = {}) {
  const palletMap = {};
  ...
  for (const row of rows) {
    ...
    validRows.push({
      ...
      cost20ftData: cost20ftMap[row.lane] || null,
