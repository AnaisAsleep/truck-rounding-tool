...
export async function parseNeedsFile(arrayBuffer, isPrio4 = false) {
  ...
  const hasOriginCode = findCol(REQUIRED_HEADERS.originLocationCode) !== undefined;
  ...
  for (let i = headerRowIndex + 1; i < rawData.length; i++) {
    ...
    // pkey must match the format used in Airtable's palletization table.
    // Airtable has: origin_location_code, receiving_destination_code, sku.
    // Using origin_location_code + sku for now — sample Airtable pkeys shown in debug.
    const pkey = `${originLocationCode}-${sku}`;
