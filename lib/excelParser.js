...
// Accepted column names for the constraint cap (Column P)
const CONSTRAINT_ALIASES = ['P', 'p', 'Constraint', 'constraint', 'Max Qty', 'max qty', 'Max', 'max', 'cap', 'Cap'];
...
export async function parseNeedsFile(arrayBuffer, isPrio4 = false) {
  ...
  // Find constraint column
  const findConstraintCol = () => {
    for (const alias of CONSTRAINT_ALIASES) {
      const idx = colIndex[alias] ?? colIndex[alias.toLowerCase()];
      if (idx !== undefined) return idx;
    }
    return undefined;
  };
  const constraintColIdx = findConstraintCol();

  // Override findCol for special columns
  ...
  for (let i = headerRowIndex + 1; i < rawData.length; i++) {
    ...
    const constraint = parseNum(getCell(row, REQUIRED_HEADERS.constraint));

    // Apply constraint cap (Column P): fill P1 first, then P2, then P3
    ...
    if (isPrio4) {
      ...
      prio1 = Math.min(rawPrio1, remaining); remaining -= prio1;
      prio2 = Math.min(rawPrio2, remaining); remaining -= prio2;
