...
export default function ReviewStep({ roundingResults, unmatchedRows = [], onConfirm, onBack }) {
  ...
  const [allBorderlineExpanded, setAllBorderlineExpanded] = useState(true);

  const borderlineTrucksReview = borderlineTrucks.map(t => ({ ...t, reviewType: 'borderline' }));
  const autocutTrucksReview    = cutTrucks.map(t => ({ ...t, reviewType: 'autocut' }));
  const reviewTrucks = [...borderlineTrucksReview, ...autocutTrucksReview];

  const setTruckField = (vsn, field, value) =>
    setTruckDecisions(prev => ({ ...prev, [vsn]: { ...prev[vsn], [field]: value } }));

  // Borderline trucks require explicit choice; auto-cut trucks default to 'cut'
  const getTruck = (vsn) => {
    if (truckDecisions[vsn]) return truckDecisions[vsn];
    const truck = reviewTrucks.find(t => t.vendorShipmentNumber === vsn);
    return { action: truck?.reviewType === 'autocut' ? 'cut' : null, luReason: null, rootCause: null };
  };
  ...
  const borderlineDone = borderlineTrucksReview.filter(t => isTruckDone(t.vendorShipmentNumber)).length;
  ...
  const toggleAllBorderline = () => {
    const next = !allBorderlineExpanded;
    setAllBorderlineExpanded(next);
    const patch = {};
    borderlineTrucksReview.forEach(t => { patch[t.vendorShipmentNumber] = next; });
    setExpandedTrucks(prev => ({ ...prev, ...patch }));
  };
    ...
    <div className="max-w-4xl">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
        ...
        <div className="bg-white border border-[#e8e0db] rounded-xl px-5 py-3 shadow-card min-w-[200px]">
          ...
          <p className="text-xs text-[#8a7e78] mt-1">Updates as you keep or cut trucks</p>
        </div>
      </div>

      {/* ── Borderline section ── */}
      {borderlineTrucksReview.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold uppercase tracking-widest text-amber-700">Borderline — needs review</span>
              <span className="text-xs bg-amber-100 text-amber-700 font-semibold px-1.5 py-0.5 rounded">{borderlineDone}/{borderlineTrucksReview.length}</span>
            </div>
            <button
              onClick={toggleAllBorderline}
              className="text-xs text-[#8a7e78] hover:text-[#403833] transition-colors"
            >
              {allBorderlineExpanded ? 'Collapse all' : 'Expand all'}
            </button>
          </div>
          <div className="bg-white border border-amber-200 rounded-xl shadow-card overflow-hidden divide-y divide-[#f0ebe8]">
            {borderlineTrucksReview.map(truck => (
              <TruckRow
                key={truck.vendorShipmentNumber}
                truck={truck}
                decision={getTruck(truck.vendorShipmentNumber)}
                done={isTruckDone(truck.vendorShipmentNumber)}
                skuAdditions={truckAdditions[truck.vendorShipmentNumber] || {}}
                isExpanded={expandedTrucks[truck.vendorShipmentNumber] !== false}
                onToggleExpand={() => toggleExpand(truck.vendorShipmentNumber)}
                onSetField={(field, val) => setTruckField(truck.vendorShipmentNumber, field, val)}
                onSetSkuAdd={(sku, patch) => setSkuAdd(truck.vendorShipmentNumber, sku, patch)}
                getSkuAdd={(sku) => getSkuAdd(truck.vendorShipmentNumber, sku)}
              />
            ))}
          </div>
        </div>
      ...
      {/* ── Auto-cut section ── */}
      {autocutTrucksReview.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs font-semibold uppercase tracking-widest text-[#8a7e78]">Auto-cut — override if needed</span>
            <span className="text-xs bg-[#f0ebe8] text-[#8a7e78] font-semibold px-1.5 py-0.5 rounded">{autocutTrucksReview.length}</span>
          </div>
          <div className="bg-white border border-[#e8e0db] rounded-xl shadow-card overflow-hidden divide-y divide-[#f0ebe8]">
            {autocutTrucksReview.map(truck => (
              <TruckRow
                key={truck.vendorShipmentNumber}
                truck={truck}
                decision={getTruck(truck.vendorShipmentNumber)}
        ...
        <div className="flex items-center gap-3">
          {!allTrucksDone && (
            <span className="text-xs text-amber-600">
              {borderlineTrucksReview.filter(t => !isTruckDone(t.vendorShipmentNumber)).length > 0
                ? `${borderlineTrucksReview.filter(t => !isTruckDone(t.vendorShipmentNumber)).length} borderline truck${borderlineTrucksReview.filter(t => !isTruckDone(t.vendorShipmentNumber)).length !== 1 ? 's' : ''} need a decision`
                : `${reviewTrucks.length - trucksDecided} truck${reviewTrucks.length - trucksDecided !== 1 ? 's' : ''} still pending`
              }
            </span>
          )}
          <button
...
function TruckRow({ truck, decision: d, done, skuAdditions, isExpanded, onToggleExpand, onSetField, onSetSkuAdd, getSkuAdd }) {
    ...
    <div className={`px-5 py-4 transition-colors ${!done ? 'bg-amber-50/30' : 'bg-white'}`}>
                ...
                return (
                  <tr key={li} className={`border-t border-[#f0ebe8] ${li % 2 !== 0 ? 'bg-[#fafaf8]' : 'bg-white'}`}>
                    ...
                    <td className="px-3 py-2 text-right text-[#403833]">
                      ...
                      {addedPcs > 0 && <span className="text-green-600 ml-1">+{addedPcs.toLocaleString()}</span>}
                    </td>
                    <td className="px-3 py-2 text-right text-[#8a7e78]">
                      {line.pallets?.toFixed(1)}
                      {addedPallets > 0 && <span className="text-green-600 ml-1">+{addedPallets}</span>}
                    </td>
                    ...
                    <td className="px-3 py-2">
                      {lineIsLoose ? (
                        <div className="flex items-center gap-1.5 justify-center">
                          <input
                            ...
                            onChange={e => {
                              ...
                              onSetSkuAdd(line.sku, { pcs, inputValue: e.target.value });
                            }}
                            placeholder="0"
                        ...
                        <div className="flex items-center gap-0 justify-center border border-[#e8e0db] rounded-lg overflow-hidden w-fit mx-auto">
                          <button
                            onClick={() => onSetSkuAdd(line.sku, { pallets: Math.max(0, add.pallets - 1) })}
                            disabled={add.pallets === 0}
                            className="px-2 py-1 text-[#8a7e78] hover:bg-[#fafaf8] disabled:opacity-30 transition-colors font-medium"
                          >−</button>
                          <span className="px-2.5 text-xs font-semibold text-[#403833] min-w-[1.5rem] text-center">{add.pallets}</span>
                          <button
                            onClick={() => onSetSkuAdd(line.sku, { pallets: add.pallets + 1 })}
                            className="px-2 py-1 text-[#8a7e78] hover:bg-[#fafaf8] transition-colors font-medium"
                          >＋</button>
                        </div>
                      )}
                    </td>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {ACTIONS.filter(opt => {
          ...
          <button
            key={opt.value}
            onClick={() => onSetField('action', opt.value)}
      ...
      {(d.action === 'keep' || d.action === '20ft') && (
        <select
          value={d.luReason || ''}
          onChange={e => onSetField('luReason', e.target.value)}
